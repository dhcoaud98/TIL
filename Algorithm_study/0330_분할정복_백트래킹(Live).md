...
* 트리는 싸이클이 없는 무향 연결그래프이다. 두 노드(or 정점) 사이에는 유일한 경로가 존재한다. 각 노드는 최대한 하나의 부모 노드가 존재할 수 있다. 각 노드는 자식 노드가 없거나 하나 이상이 존재할 수 있다. 
* 비선형 구조 : 원소들간에 1:n 관계를 가지는 자료구조/ 원소들 간에 계층 관계를 가지는 계층형 자료 구조  
