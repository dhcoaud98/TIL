          ...
          <v-container>
            <v-row>
              <v-col cols="12" class="d-flex justify-space-between">
                <h3 class="mt-2 ms-2">내가 팔로우한 사람들</h3>
            ...
            <v-row>
              <v-col class="pa-0">
                <v-container class="pt-0" max-height="400px">
                  <div v-for="(myFollowingElem,idx) in myFollowingList" :key="idx">
                    <v-btn @click="profileEvent(myFollowingElem.id)" plain x-large class=" purple--text text--darken-1 d-flex justify-space-between bottom-lined" width="100%">
                      <h4>유저이름: {{myFollowingElem.name}}</h4>
                      <h4>포인트: {{myFollowingElem.point}}</h4>
                    </v-btn>
                    <v-divider></v-divider>
                  </div>
                </v-container>
...
export default {
  ...
  created() {
    axios({
        ...
        .then(res => {
          this.myFollowingList = res.data.content
